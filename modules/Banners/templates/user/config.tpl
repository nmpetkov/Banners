...
<h2>{gt text="Currently active banners for %s." tag1=$name}</h2>
...
<table width="100%" border="1">
	<tr>
     ...
     <tr>
		...
		<td><a href="{modurl modname="Banners" type="user" func="emailstats" login="$login" pass="$pass" cid="$cid" bid=""}{$banners[banners].bid|safetext}">{gt text="Email Stats"}</a></td>
	 </tr>
	{/section}
</table>
...
<div>
	...
	<input type="hidden" name="login" value="{$login|safetext}" />
	<input type="hidden" name="pass" value="{$pass|safetext}" />
